...
// Reuse Step Components from user edit flow
import StepTypeSelection from "@/app/(dashboard)/user/properties/edit/[slug]/components/StepTypeSelection";
import StepBasicInfo from "@/app/(dashboard)/user/properties/edit/[slug]/components/StepBasicInfo";
import StepSpecifications from "@/app/(dashboard)/user/properties/edit/[slug]/components/StepSpecifications";
import StepAmenities from "@/app/(dashboard)/user/properties/edit/[slug]/components/StepAmenities";
import StepMediaUpload from "@/app/(dashboard)/user/properties/edit/[slug]/components/StepMediaUpload";
import StepKycVerification from "@/app/(dashboard)/user/properties/edit/[slug]/components/StepKycVerification";
import StepReviewPublish from "@/app/(dashboard)/user/properties/edit/[slug]/components/StepReviewPublish";
...
export default function PartnerPropertyEditWizard() {
    ...
    const saveTempData = (data) => {
        sessionStorage.setItem(`partnerPropertyEditTempData_${propertySlug}`, JSON.stringify(data));
    };

    const clearTempData = () => {
        sessionStorage.removeItem(`partnerPropertyEditTempData_${propertySlug}`);
        sessionStorage.removeItem(`partnerPropertyEditFormData_${propertySlug}`);
    };
    ...
    useEffect(() => {
        const loadPropertyForEdit = async () => {
            ...
            try {
                ...
                const tempData = sessionStorage.getItem(`partnerPropertyEditTempData_${propertySlug}`);
                if (tempData) {
    ...
    useEffect(() => {
        return () => {
            sessionStorage.removeItem(`partnerPropertyEditData_${propertySlug}`);
            clearTempData();
        };
    }, [propertySlug]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (Object.keys(changes).length === 0 && !fieldsToAlwaysInclude.some(field => formData[field] !== undefined)) {
                ...
                router.push(`/partner/properties/${originalProperty._id}`);
                return;
            }
            ...
            // Redirect to partner property details
            router.push(`/partner/properties/${originalProperty._id}?updated=true`);
        } catch (error) {
    ...
    const handleSaveDraft = () => {
        sessionStorage.setItem(`partnerPropertyEditData_${propertySlug}`, JSON.stringify(formData));
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            >
                ...
                <div className="mb-6">
                    <button
                        onClick={() => router.push(`/partner/properties/${originalProperty._id}`)}
